...
const NavbarDropdown = ({
  ...
  return (
    ...
    >
      ...
      {/* <div
        className={`absolute left-1/2 -translate-x-1/2 top-full z-[60] transition-all duration-200 ease-in-out ${
          isOpen
            ? "opacity-100 translate-y-0 pointer-events-auto"
            : "opacity-0 -translate-y-2 pointer-events-none"
        }`}
      >
        <div className="bg-white border border-gray-200/50 shadow-2xl rounded-xl overflow-hidden backdrop-blur-xl w-64 mt-1">
          {items.map((item, index) => (
            <div
              key={index}
              className="hover:bg-gradient-to-r hover:from-green-50 hover:to-emerald-50 focus:bg-gradient-to-r focus:from-green-50 focus:to-emerald-50 transition-all duration-200 cursor-pointer px-4 py-3 text-gray-700 hover:text-green-700 font-medium group border-b border-gray-100 last:border-b-0"
              onClick={() => {
